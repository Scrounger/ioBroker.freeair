...
import type { JsonConfigAutocompleteSendTo, myCommonState, requestData } from './lib/myTypes.js';
...
class Freeair extends utils.Adapter {
	...
	private async parseUrl(data: requestData) {
	...
	async setDeviceConnectionStatus(serialNo: string, isConnected: boolean): Promise<void> {
		...
		try {
			this.isConnected = isConnected;
			await this.setState(`${serialNo}.${(tree.FreeAirDevice.get().isOnline as myCommonState).id}`, isConnected, true);
